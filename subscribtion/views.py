...
#     if is_upgrade:
#         message = (
#             f"Hello byteCub Team! I want to UPGRADE my subscription:\n"
...
#     else:
#         message = (
#             f"Hello byteCub Team! I want to subscribe to:\n"
#             f"• Plan: {plan.name}\n"
...
def subscription_plans(request):
    ...
    user_subscription, created = UserSubscription.objects.get_or_create(
        user=request.user,
        defaults={'plan': None, 'status': 'expired'}
    )
...
def choose_plan(request, plan_id):
    ...
    if user_subscription.status == 'active' and user_subscription.plan == plan:
        messages.warning(request, f"You already have an active {plan.name} subscription.")
        return redirect('my_subscription')
    
    # Check if user has a pending payment
    if user_subscription.status == 'pending':
        messages.warning(request, "You have a pending payment. Please complete it before selecting a new plan.")
        return redirect('my_subscription')
    ...
    if user_subscription.is_active():
        if plan.price_usd > user_subscription.plan.price_usd:
            is_upgrade = True
            upgrade_data = user_subscription.calculate_upgrade_price(plan)
            upgrade_price_usd = upgrade_data['usd']
            upgrade_price_cfa = upgrade_data['cfa']
        else:
            messages.warning(request, "You can only upgrade to a higher-tier plan.")
    ...
    user_subscription.is_upgrade = is_upgrade
    user_subscription.upgrade_price_usd = upgrade_price_usd if is_upgrade else None
    user_subscription.upgrade_price_cfa = upgrade_price_cfa if is_upgrade else None
    
    # Store previous plan info if it's an upgrade
    if is_upgrade and user_subscription.is_active():
        user_subscription.previous_plan_name = user_subscription.plan.name
        user_subscription.previous_plan_price = user_subscription.plan.price_usd
    ...
    if is_upgrade:
        message = (
            ...
            f"• From: {user_subscription.previous_plan_name} (${user_subscription.previous_plan_price})\n"
            f"• To: {plan.name} (${plan.price_usd})\n"
            f"• Upgrade cost: ${upgrade_price_usd:.2f} USD / {upgrade_price_cfa:.0f} CFA Frs\n"
            f"• My email: {request.user.email}"
        )
...
def subscription_activation(request):
    ...
    if search_query:
        pending_subscriptions = pending_subscriptions.filter(
            ...
            # Q(subscription_id=search_query) |
            Q(plan__price_cfa=search_query)
        )
        active_subscriptions = active_subscriptions.filter(
            Q(user__email__icontains=search_query) | 
            # Q(subscription_id=search_query) |
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # If this is an upgrade, add to history
                if subscription.is_upgrade and subscription.previous_plan_name:
                    subscription.add_upgrade_history(
                        from_plan=subscription.plan,  # This would need adjustment
                        to_plan=subscription.plan,
                        upgrade_price=subscription.upgrade_price_usd,
